...
RSpec.configure do |c|
  ...
  c.before :suite do
    ...
    hosts.each do |host|
      ...
      copy_module_to(host, :source => proj_root, :module_name => 'rook')
...
EOS
        ...
        on(host, "cd  /etc/puppetlabs/code/environments/production/modules/kubernetes;bundle install", acceptable_exit_codes: [0]).stdout
        # Creating certs
        on(host, "export PATH=$PATH:/usr/local/go/bin;export PATH=$PATH:/root/go/bin;cd  /etc/puppetlabs/code/environments/production/modules/kubernetes/tooling;./kube_tool.rb -f kubernetes -i #{vmipaddr} -b #{vmipaddr} -e \"etcd-#{vmhostname}=http://#{vmipaddr}:2380\" -t \"%{::ipaddress_ens160}\" -a \"%{::ipaddress_ens160}\" -d true", acceptable_exit_codes: [0]).stdout
        create_remote_file(host, "/etc/hosts", hosts_file)
        create_remote_file(host, "/tmp/nginx.yml", nginx)
        on(host, 'cp /etc/puppetlabs/code/environments/production/modules/kubernetes/tooling/kubernetes.yaml /etc/puppetlabs/code/environments/production/hieradata/common.yaml', acceptable_exit_codes: [0]).stdout
    end
  end
end
